...
use crate::descriptor::FixedArrayKind;
...
impl<'a, 'b> JsBuilder<'a, 'b> {
    ...
    fn pop(&mut self) -> String {
        println!("Stack before pop: {:?}", self.stack);
    ...
    match instr {
        ...
        | Instruction::DeferFree { .. } => {
            ...
            if invoc.defer() {
                ...
                println!("Not deferred, params: {}", params);
        ...
        Instruction::WasmToFixedArray {
            ...
            let free = js.cx.export_name_of(*free);
            let mut to_ret = format!("v{}", js.tmp());
            js.prelude(&format!("var {to_ret} = {f}({ptr}, {length})"));
            if let FixedArrayKind::RustStruct(class) = kind {
                js.cx.require_class_wrap(class);
                let item = format!("i{}", js.tmp());
                let new_to_ret = format!("v{}", js.tmp());
                js.prelude(&format!("var {new_to_ret} = [];"));
                js.prelude(&format!(
                    "for (const {item} of {to_ret}) {{
                        {new_to_ret}.push({class}.__wrap({item}));
                     }}",
                ));
                to_ret = new_to_ret;
            }
        ...
        Instruction::FixedArrayToWasm {
            ...
            let input = js.pop();
            let func = js.cx.pass_array_to_wasm_function(kind.clone(), *mem)?;
            let malloc = js.cx.export_name_of(*malloc);
            let i = js.tmp();
